...
def count_zero_crossings(duration_ms):
    ...
    # シリアルモニタにカウントを出力
    uart.write('Zero crossings count: {}\n'.format(count))
...
# メインループ
while True:
    zero_crossings = count_zero_crossings(ZERO_CROSSING_DURATION_MS)
    uart.write('Zero crossings: {}\n'.format(zero_crossings))
    ...
    if command_mode:
        ...
        # クリック数を数える
        print("command mord ")
        while time.time() - start_time < 3:  # 2秒間クリックを待つ
            if count_zero_crossings(ZERO_CROSSING_DURATION_MS) >= CLICK_THRESHOLD_FREQUENCY:
                click_count += 1
                time.sleep(DEBOUNCE_TIME_MS / 1000)  # デバウンス
